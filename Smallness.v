...
(* i < k, j <= k. *)
Global Instance ishprop_issmall@{i j k} `{Univalence} (X : Type@{j}) : IsHProp (IsSmall@{i j} X).
Proof.
...
(* i <= j *)
Definition issmall_in@{i j} (X : Type@{i}) : IsSmall@{i j} X
...
(* i < k, j <= k. *)
Fixpoint IsLocallySmall@{i j k} (n : nat) (X : Type@{j}) : Type@{k}
  := match n with
    | 0%nat => IsSmall@{i j} X
...
(* i < k <= u, j <= k, j < u. *)
Definition ishprop_islocally_small@{i j k u} `{Univalence} (n : nat) (X : Type@{j})
  : IsHProp@{k} (IsLocallySmall@{i j k} n X).
Proof.
  revert n X.
  (* Can't use [induction n], as we need to control the universe variable. *)
  nrapply nat_rect@{u}; exact _.
Defined.

(* i <= j <= k, i < k. *)
Definition islocally_small_in@{i j k} (n : nat) (X : Type@{i}) : IsLocallySmall@{i j k} n X.
Proof.
  revert X.
  induction n; intro X.
  - apply issmall_in.
...
(* i < k, {j1,j2} <= k <= u, {j1,j2} < u. *)
(* Again, we use [nat_rect] rather than [induction n] to control the universe variable. *)
Definition islocally_small_equiv_islocally_small@{i j1 j2 k u} (n : nat) {A : Type@{j1}} {B : Type@{j2}}
...
(* i < k <= u, j <= k, j < u. *)
Definition sigma_closed_islocally_small@{i j k u} (n : nat) {A : Type@{j}}
           (B : A -> Type@{j}) (lsA : IsLocallySmall@{i j k} n A)
           (lsB : forall a, IsLocallySmall@{i j k} n (B a))
  : IsLocallySmall@{i j k} n { a : A & B a }.
Proof.
  revert n A B lsA lsB.
  rapply nat_rect@{u}.
  - exact @sigma_closed_issmall.
  - intros n IHn A B lsA lsB x y.
...
(* i < k <= u, j <= k, j < u. *)
Definition islocally_small_small@{i j k u} (n : nat)
...
(* i < k, j <= k. *)
Definition jc_surjection@{i j k} `{Funext} (A : Type@{i}) (X : Type@{j})
...
(* i < k < u, j <= k, very slightly stronger than earlier constraints. *)
Definition issmall_n_image@{i j k u} `{Univalence} (n : trunc_index) {A : Type@{i}} {X : Type@{j}}
